...
#include "../src/network/poll_tcp_server.hpp"

using namespace std;
using namespace soda;

void recv_cb(PollTCPServer &s, int32_t fd, const string &addr, uint16_t port, const void *data, size_t data_size)
{
...
void conn_cb(PollTCPServer &s, int32_t fd, const string &addr, uint16_t port)
{
    cout << addr << ":" << port << " fd:" << fd << " connected " << endl;
}

void disconn_cb(PollTCPServer &s, const string &addr, uint16_t port)
{
    cout << addr << ":" << port << " disconnected " << endl;
}

void send_msg(PollTCPServer &s)
{
    string input;
    int32_t fd = -1;
    while (1)
